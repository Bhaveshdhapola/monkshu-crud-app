...
const simpleInsert = (tableName, queryParams) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // const query = `INSERT INTO ${tableName} (name, age) VALUES (?,?)`;
            // Use the connection
            const queryStats = connection.query(query, queryParams, (error, results) => {
                // When done with the connection, release it.
                releaseConnection(connection);
 ...
 * This function can be used to perform update only, after query connection get closed
 *
 * Sample query = `DELETE FROM users WHERE userId=?`
 * Sample queryParams = [1]
 * 
 * @param {String} query - SQL Query to be processed
 * @param {Array} queryParams - Array of query parameters including where values
 * @returns {Promise<boolean>} - returns Boolean else throws Error
...
module.exports = {
    ...
    consecutiveSelect, simpleInsert, consecutiveInsert, simpleUpdate, consecutiveUpdate, simpleDelete
};